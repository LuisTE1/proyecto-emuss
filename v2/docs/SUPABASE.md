...
La configuración de Auth alojada está en `supabase/hosted/supabase/config.toml`. Incluye las URLs de retorno locales y vencimiento OTP de diez minutos. Para aplicar cambios revisados:
...
Por indicación del usuario se creó `u201916314@upc.edu.pe` con el rol activo `SUPER_ADMIN`. La contraseña inicial está en `.env.owner`, excluido de Git, bajo `EMUSS_ADMIN_PASSWORD`. Acceso: http://127.0.0.1:5173/admin/login con ese correo y contraseña. El correo identifica la cuenta de EMUSS; no se configuró inicio de sesión OAuth con GitHub.
